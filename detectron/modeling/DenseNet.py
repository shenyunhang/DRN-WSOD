...
def add_stage(
    ...
    if 'conv2' in prefix:
        blob_in = model.MaxPool(blob_in, prefix + '_pool', kernel=3, pad=1, stride=2)
    else:
        blob_in = model.AveragePool(blob_in, prefix + '_pool', kernel=2, pad=0, stride=2)
...
def add_DenseNet_convX_body(model, block_counts, dim_in, dim_inner, group_rate):
    ...
    s, dim_in = add_stage(model, 'conv2', p, n1, dim_in, group_rate, dim_inner, 1)
    if freeze_at == 2:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'conv3', s, n2, dim_in, group_rate, dim_inner, 1
    )
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'conv4', s, n3, dim_in, group_rate, dim_inner, 1
    )
    if freeze_at == 4:
        model.StopGradient(s, s)
